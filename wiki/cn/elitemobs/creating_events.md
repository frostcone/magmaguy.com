...
此处的自定义事件是指游戏中随机发生的事件，无论是基于玩家操作还是具有定时触发器。该插件预加载了许多自定义事件，例如 Balrog 事件、Kraken 事件和 Treasure Goblin 事件等等。

此系统称为**自定义**事件，因为事件不仅可以自定义，还可以从头开始创建。以下是如何创建和自定义您自己的事件的指南。

# 常见配置设置

<div align="center">

以下设置可以/应该用于操作事件和定时事件。
...
*请注意，`BREAK_BLOCK`、`FISH` 和 `TILL_SOIL` 是[操作事件](#action-events)，而 `TIMED` 是[定时事件](#timed-events)*
...
startMessage: "酷炫的事件即将开始！"
endMessage: "酷炫的事件即将结束！"
eventStartCommands:
- say 事件已开始！
eventEndCommands:
- say 事件已结束！
...
设置要检查操作是否设置为 `BREAK_BLOCK` 的材质列表。

| 键        |       值        | 默认值 |
|-----------|:---------------:|:------:|
| `breakableMaterials` | [材质](https://hub.spigotmc.org/javadocs/spigot/org/bukkit/Material.html) 列表 |  无   |
...
设置在破坏煤矿石方块时运行事件的 0.1% 几率，假设事件类型为 `BREAK_BLOCK`。
...
设置事件使用的[自定义生成]($language$/elitemobs/creating_spawns.md)，该生成定义了首领可以生成的位置。
