# 什么是 LibsDisguises？

LibsDisguises 是一个插件，允许服务器将任何实体伪装成任何其他类型的实体。例如，你可以将玩家伪装成羊，或者将羊伪装成玩家。

LibsDisguises 有一个[免费](https://www.spigotmc.org/resources/libs-disguises-free.81/)
版本和一个[高级](https://www.spigotmc.org/resources/libs-disguises.32453/)版本。LibsDisguises 由其他开发者创建和维护，所以请不要在
EliteMobs 的支持部分寻求它的帮助。

# 如何将实体伪装成玩家？

要将自定义 Boss（或区域 Boss，甚至 NPC！）伪装成玩家，你只需在 Boss 配置文件中添加以下行：

```yml
disguise: player:name
```

例如，如果你想将一个 Boss 伪装成我的玩家皮肤，配置行将如下所示：

```yml
disguise: player:magmaguy
```

# 如何将实体伪装成玩家皮肤，但不一定是玩家？

这是将实体伪装成玩家最灵活的方式，因为它允许管理员使用在线皮肤，甚至创建自己的皮肤来伪装 Boss。它也稍微复杂一些。

为简单起见，这里列出的示例假设你将使用在线可用的皮肤。Libsdisguises
提供了不同的反序列化皮肤的方法，包括将你自己的自定义皮肤放入其文件夹中的方法。如果你不想使用已经在线可用的皮肤，请查看他们的文档。

### 1. 设置伪装

对于伪装，你将需要执行类似以下的操作：

```yaml
disguise: custom:your_unique_skin_name
```

### 2. 生成伪装数据

这是棘手的部分。首先创建以下配置条目：

```yaml
customDisguiseData: player your_unique_skin_name setskin
```

*至关重要的是，你在伪装中设置的 your_unique_skin_name 名称必须与 customDisguiseData 中的名称匹配！*

然后，你需要反序列化玩家皮肤。对于在线公开可用的皮肤来说，最简单的方法是使用 LibsDisguises 的 grabskin 命令。

假设你想获取[这个皮肤](https://www.minecraftskins.com/skin/18512841/buff-villager/)。

为此，你需要运行命令 `/grabskin https://www.minecraftskins.com/uploads/skins/2021/07/28/buff-villager-18512841.png?v437`

*
*注意：你需要提供指向皮肤文件的直接链接，而不是指向包含皮肤的页面。请注意链接指向的位置。在 [skindex](https://www.minecraftskins.com/)
中，你可以通过按每个皮肤右下角的“图像链接”按钮来获取直接链接。**

成功执行此操作后，LibsDisguises 将为你提供大约 3-6 个显示“单击以复制”的数字。如果你单击这些数字，将出现一长串数字和字母。你要确保从
1 开始，并按顺序执行所有数字。要复制它们，请按 ctrl-a，然后按 ctrl-c，然后使用 ctrl-v 将它们粘贴到文件中。完成后，它应该类似于这样：

```yaml
customDisguiseData: player your_unique_skin_name setskin {"id":"d1fbb77f-b184-4718-b0a2-c7ae866798d3","name":"Unknown","properties":[{"name":"textures","value":"","signature":""}],"legacy":false}
```

就这样！`customDisguiseData` 可以与任何反序列化的皮肤代码一起使用，因此如果你找到在 LibsDisguises 中生成相同代码的其他方法，你可以在此处使用它。

customDisguiseData 的优点是它是自包含的，这意味着你不必依赖在线玩家、拥有特定皮肤的玩家或其他任何内容。EliteMobs
使用它来为地牢包中所有伪装的 Boss 换皮肤！

# 如何将自定义 Boss 伪装成另一种类型的实体？

要将自定义 Boss（或区域 Boss）伪装成另一种类型的实体，你只需在 Boss 配置文件中添加以下行：

```yml
disguise: ENTITY_TYPE
```

例如，如果你希望你的自定义 Boss 伪装成羊，你可以这样做：

```yml
disguise: SHEEP
```

## 如何将自定义 Boss 伪装成幼年实体？

要将自定义 Boss（或区域 Boss）伪装成幼年实体，你只需在 Boss 配置文件中添加以下行：

```yml
disguise: HOGLIN:baby
```

正如你在这个例子中看到的，我们的 Boss 将被伪装成一个幼年疣猪兽。

# 如何使用自定义伪装来伪装实体？

要将自定义 Boss（或区域 Boss）伪装成自定义 LibsDisguises 实体，你需要先创建 LibsDisguises 伪装。

完成后，正确的语法是：

```yml
disguise: custom:you_custom_disguise_name
```

# 有效的伪装

没有官方的有效实体类型列表，但以下是一个非官方列表，在编写本文时（2020 年 8 月 3 日）应该可以正常工作

<details>

<summary><b>展开列表</b></summary>

```
    ARMOR_STAND
    ARROW
    BAT
    BLAZE
    BOAT
    CAT
    CAVE_SPIDER
    CHICKEN
    COD
    COW
    CREEPER
    DOLPHIN
    DONKEY
    DRAGON_FIREBALL
    DROWNED
    DROPPED_ITEM
    EGG
    ELDER_GUARDIAN
    ENDER_CRYSTAL
    ENDER_DRAGON
    ENDER_PEARL
    ENDER_SIGNAL
    ENDERMAN
    ENDERMITE
    EVOKER
    EVOKER_FANGS
    EXPERIENCE_ORB
    FALLING_BLOCK
    FIREBALL
    FIREWORK
    FISHING_HOOK
    FOX
    GHAST
    GIANT
    GUARDIAN
    HOGLIN
    HORSE
    HUSK
    ILLUSIONER
    IRON_GOLEM
    ITEM_FRAME
    LLAMA
    LLAMA_SPIT
    LEASH_HITCH
    MAGMA_CUBE
    MINECART
    MINECART_CHEST
    MINECART_COMMAND
    MINECART_FURNACE
    MINECART_HOPPER
    MINECART_MOB_SPAWNER
    MINECART_TNT
    MODDED_MISC
    MODDED_LIVING
    MULE
    MUSHROOM_COW
    OCELOT
    PAINTING
    PANDA
    PARROT
    PHANTOM
    PIG
    PIG_ZOMBIE
    PIGLIN
    PILLAGER
    PLAYER
    POLAR_BEAR
    PRIMED_TNT
    PUFFERFISH
    RABBIT
    RAVAGER
    SALMON
    SHEEP
    SHULKER
    SHULKER_BULLET
    SILVERFISH
    SKELETON
    SKELETON_HORSE
    SLIME
    SMALL_FIREBALL
    SNOWBALL
    SNOWMAN
    SPECTRAL_ARROW
    SPLASH_POTION
    SQUID
    STRAY
    STRIDER
    THROWN_EXP_BOTTLE
    TIPPED_ARROW
    TRIDENT
    TRADER_LLAMA
    TROPICAL_FISH
    TURTLE
    VEX
    VILLAGER
    VINDICATOR
    WANDERING_TRADER
    WITCH
    WITHER
    WITHER_SKELETON
    WITHER_SKULL
    WOLF
    ZOGLIN
    ZOMBIE
    ZOMBIE_HORSE
    ZOMBIE_VILLAGER
    ZOMBIFIED_PIGLIN
```

</details>
