...
*Note que `BREAK_BLOCK`, `FISH` e `TILL_SOIL` são [Eventos de Ação](#eventos-de-ação) e `TIMED` é [Eventos Cronometrados](#eventos-cronometrados)*
...
startMessage: "Evento fixe está a começar!"
endMessage: "Evento fixe está a terminar!"
eventStartCommands:
...
Define uma chance de 0,1% de executar o evento quando um bloco de minério de carvão é quebrado, assumindo que o tipo de evento seja `BREAK_BLOCK`.
