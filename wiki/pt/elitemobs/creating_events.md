...
Eventos Personalizados aqui referem-se a eventos que ocorrem aleatoriamente no jogo, seja com base em ações do jogador ou com um gatilho temporizado. O plugin vem pré-carregado com vários Eventos Personalizados, como o evento Balrog, o evento Kraken e os eventos Treasure Goblin, para citar alguns.

Este sistema é chamado de Eventos **Personalizados** porque os eventos não só podem ser personalizados, mas também criados do zero. O que se segue é um guia sobre como criar e personalizar os seus próprios eventos.

# Definições de configuração comuns

<div align="center">

As seguintes definições podem/devem ser usadas tanto para eventos de ação quanto para eventos temporizados.
...
*Note que `BREAK_BLOCK`, `FISH` e `TILL_SOIL` são [Eventos de Ação](#action-events) e `TIMED` é [Eventos Temporizados](#timed-events)*
...
startMessage: "O evento fixe está a começar!"
endMessage: "O evento fixe está a terminar!"
eventStartCommands:
...
Eventos que têm uma chance de ocorrer quando uma ação específica, como quebrar um bloco ou pescar, acontece.
...
Define a lista de materiais a verificar se a ação está definida como `BREAK_BLOCK`.
...
Eventos Temporizados são eventos que acontecem em intervalos de tempo configuráveis. Assim que o cooldown para um evento temporizado termina, um evento temporizado aleatório é escolhido da lista de eventos temporizados com base no peso do evento.
...
| Chave       |        Valores         | Padrão |
|-----------|:----------------------:|:------:|
| `spawnType` | [Nome do Ficheiro](#filename) | nenhum |
...
*Note que os eventos são enfileirados, o que significa que só começarão assim que as condições definidas no `customSpawn` forem cumpridas.*
